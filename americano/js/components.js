                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-2 text-sm text-white/70">
                                    ...
                                    <span>•</span>
                                    <span>${state.courtCount} court${state.courtCount > 1 ? 's' : ''}</span>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-2">
                            ${canEdit ? '<span class="text-xs bg-amber-500/20 text-amber-200 px-2 py-1 rounded-full">Organiser</span>' : ''}
                            <button onclick="showShareModal()" class="p-2 hover:bg-white/10 rounded-xl transition-colors">
                                <span class="text-xl">📤</span>
                            </button>
                            ${!canEdit ? `<button onclick="showOrganiserLoginModal()" class="p-2 hover:bg-white/10 rounded-xl transition-colors"><span class="text-xl">🔑</span></button>` : ''}
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border-b border-gray-100 px-4 py-2">
                <div class="max-w-5xl mx-auto">
                    ...
                    <div class="h-2 bg-gray-100 rounded-full overflow-hidden">
                        <div class="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-300" style="width: ${totalMatches > 0 ? (completedMatches / totalMatches * 100) : 0}%"></div>
                    </div>
                </div>
            </div>
...
/**
 * Render fixtures tab with rounds
 */
function renderFixturesTab() {
    ...
    const roundButtons = `
        ...
        ${rounds.map((round, idx) => {
            const isComplete = round.matches.every((_, matchIdx) => {
                const score = state.getScore(idx, matchIdx);
            ...
            return `
                <button onclick="state.selectedRound = ${idx}; render();" 
                    class="round-btn w-10 h-10 rounded-xl font-semibold text-sm transition-all ${isSelected ? 'bg-blue-500 text-white shadow-lg' : isComplete ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}">
                    ${idx + 1}
                </button>
            `;
        }).join('')}
    ...
    // Build round cards
    const roundCards = roundsToShow.map(round => renderRoundCard(round)).join('');
...
function renderRoundCard(round) {
    const canEdit = state.canEdit();
    const allComplete = round.matches.every((_, matchIdx) => {
        const score = state.getScore(round.index, matchIdx);
        return score.team1 !== null && score.team2 !== null;
    });
    ...
    return `
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div class="px-4 py-3 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <div class="flex items-center gap-3">
                    <div class="font-bold text-gray-800">Round ${round.index + 1}</div>
                    ${allComplete ? '<span class="text-green-600 text-sm">✓ Complete</span>' : ''}
                </div>
                ${round.resting.length > 0 ? `
                    <div class="resting-badge">
                        ⏸️ Resting: ${round.resting.map(p => getPlayerName(p)).join(', ')}
                    </div>
                ` : ''}
            </div>
            
            <!-- Matches Grid -->
            <div class="p-4 grid grid-cols-1 ${state.courtCount > 1 ? 'lg:grid-cols-2' : ''} gap-4">
                ${round.matches.map((match, matchIdx) => renderMatchCard(round.index, matchIdx, match)).join('')}
            </div>
        </div>
    `;
}
 ...
 */
function renderMatchCard(roundIndex, matchIndex, match) {
    const canEdit = state.canEdit();
    const score = state.getScore(roundIndex, matchIndex);
    ...
    const scoreDisplay = canEdit ? `
        <input type="number" min="0" 
            ...
            onchange="state.updateScore(${roundIndex}, ${matchIndex}, 'team1', this.value); render();" />
        ...
        <input type="number" min="0" 
            ...
            onchange="state.updateScore(${roundIndex}, ${matchIndex}, 'team2', this.value); render();" />
    ` : `
        <span class="text-3xl font-bold text-gray-800">${score.team1 !== null ? score.team1 : '—'}</span>
        <span class="text-gray-400 text-2xl font-semibold">:</span>
        <span class="text-3xl font-bold text-gray-800">${score.team2 !== null ? score.team2 : '—'}</span>
    `;
    ...
    return `
        <div class="match-card ${isComplete ? 'complete' : ''}">
            <div class="px-4 py-2 flex justify-between items-center bg-gray-50">
                <div class="flex items-center gap-2">
                    <div class="font-semibold text-gray-800">${getCourtName(matchIndex)}</div>
                </div>
                ${isComplete ? '<span class="text-green-600 text-sm">✓</span>' : ''}
            </div>
            <div class="px-3 py-4">
                <div class="match-row">
                    <div class="team-stack">
                        ${team1Players.map(p => `
                            <div class="player-badge-compact ${getPlayerColorClass(p)}">
                    ...
                    ${isComplete && canEdit ? `
                        <button onclick="state.clearScore(${roundIndex}, ${matchIndex}); render();" 
...
/**
 * Render leaderboard tab (Team League style)
 */
    ...
    return `
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <table class="standings-table">
                    ...
                    <tbody>
                        ${standings.map((player, index) => `
                            <tr>
                                <td class="position ${index < 3 ? 'qualified' : ''}">${index + 1}</td>
                                <td>
                                    <div class="team-cell">
            ...
            <div class="px-4 py-3 bg-gray-50 border-t border-gray-100 text-xs text-gray-500">
                P = Played • W = Wins • L = Losses • PF = Points For • PA = Points Against • PD = Point Diff • PTS = Total Points
            </div>
        </div>
    `;
}
    ...
    return `
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">Edit court names for display</p>
            </div>
            <div class="p-4 space-y-3">
                ${state.courtNames.map((name, index) => `
                    <div class="flex items-center gap-3">
