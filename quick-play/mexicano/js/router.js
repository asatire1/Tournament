 ...
 * Handles hash-based routing for the Mexicano app
 */
...
const Router = {
    ...
    handleRoute() {
        ...
        if (!hash || hash === '/' || hash === '') {
            ...
            if (path.endsWith('/tv')) {
                ...
                this.tournamentId = path.slice(0, -3).toLowerCase();
            } else {
                this.currentRoute = this.routes.TOURNAMENT;
                this.tournamentId = path.toLowerCase();
            }
    ...
    /**
     * Generate player link for sharing (view-only)
     */
    ...
    /**
     * Generate organiser link with key (can edit)
     */
...
console.log('✅ Mexicano Router loaded');
