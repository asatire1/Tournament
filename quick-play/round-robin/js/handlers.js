// ===== ROUND ROBIN EVENT HANDLERS =====
// Simplified fork of Team League handlers.
// Round Robin = single league, no groups, no knockout.
...
function handleScore(team1Id, team2Id, team1Score, team2Score) {
    ...
    if (score1 !== null && score2 !== null) {
        ...
        setTimeout(() => renderRoundRobin(), 100);
    }
}
...
function removeTeam(teamId) {
    ...
    if (confirm(`Remove team "${team.name}"? This will clear all fixtures.`)) {
...
function generateFixtures() {
    ...
    if (state.teams.length < 4) {
        showToast('Need at least 4 teams to generate fixtures');
        return;
    }
...
function moveFixtureUp(roundIdx, matchIdx) {
    if (!state || !state.canEdit()) return;
    if (roundIdx === 0) return;

    const currentRound = state.fixtures[roundIdx];
    const prevRound = state.fixtures[roundIdx - 1];
...
function moveFixtureDown(roundIdx, matchIdx) {
    if (!state || !state.canEdit()) return;

    if (roundIdx >= state.fixtures.length - 1) return;

    const currentRound = state.fixtures[roundIdx];
    const nextRound = state.fixtures[roundIdx + 1];
...
function exportTournamentData() {
    ...
    a.href = url;
    a.download = `round-robin-${state.tournamentId}-${new Date().toISOString().split('T')[0]}.json`;
...
function showShareModal() {
    ...
    const shareButtonsHTML = typeof SocialShare !== 'undefined' ? SocialShare.getShareButtons(state.tournamentName || 'Round Robin Tournament', state.tournamentId, 'round-robin', playerLink, standings) : '';
            ...
            <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-slide-up">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div class="text-center mb-4">
                        <div class="text-lg font-bold text-gray-800">${state.tournamentName || 'Round Robin Tournament'}</div>
                        <div class="text-sm text-gray-500">Code: <span class="font-mono font-bold text-emerald-600">${state.tournamentId?.toUpperCase() || ''}</span></div>
                    </div>

                    <div class="bg-emerald-50 rounded-xl p-4">
                        <div class="flex items-center justify-between mb-2">
                            <span class="text-sm font-semibold text-emerald-800">Player Link</span>
                            <button onclick="copyToClipboard('${playerLink}'); this.textContent = 'Copied!'; setTimeout(() => this.textContent = 'Copy', 2000)" class="px-3 py-1 bg-emerald-500 hover:bg-emerald-600 text-white text-xs rounded-lg font-medium transition-colors">Copy</button>
                        </div>
                        <div class="text-xs text-emerald-600 font-mono break-all bg-white p-2 rounded-lg">${playerLink}</div>
                    </div>
                    ...
                    ${organiserLink ? `
                        <div class="bg-amber-50 rounded-xl p-4">
                            ...
                            <p class="text-xs text-amber-600 mt-2">Keep this private!</p>
                        </div>
                    ` : ''}
        ...
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onclick="if(event.target === this) closeModal()">
            <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-slide-up">
                ...
                <div class="p-6">
                    ...
                    <div class="mb-4">
                        <input type="password" id="login-passcode" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-emerald-500 focus:outline-none transition-colors text-lg text-center" placeholder="Enter passcode" autofocus onkeypress="if(event.key === 'Enter') handleOrganiserLogin()" />
                    </div>
                    ...
                    <div class="flex gap-3">
                        ...
                        <button onclick="handleOrganiserLogin()" class="flex-1 px-5 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-semibold transition-colors">Login</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden animate-slide-up">
                ...
                <div class="bg-gradient-to-r from-emerald-600 to-green-600 px-6 py-5">
                    <div class="flex items-center justify-between mb-3">
                        <h2 class="text-xl font-bold text-white">Create Round Robin</h2>
...
function renderCreateStep1() {
    return `
        <div id="wizard-content">
            ...
            <p class="text-gray-500 text-sm mb-6">Give your round robin tournament a name</p>
        ...
        <div class="flex gap-3 mt-6">
            ...
            <button onclick="createWizardNext()" class="flex-1 px-5 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-semibold transition-colors">
                Next
            </button>
        </div>
    `;
            ...
            <div class="grid grid-cols-2 gap-3">
                ${options.map(pts => `
                    <button
                        ...
                        class="p-4 rounded-xl border-2 text-center font-semibold text-lg transition-colors ${CreateWizard.maxScore === pts ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-200 hover:border-emerald-300 text-gray-700'}"
                    >
                        ${pts}
                    </button>
                `).join('')}
        ...
        <div id="wizard-content">
            ...
            <div class="bg-gray-50 rounded-xl p-4 mb-4">
                <div class="grid grid-cols-2 gap-3 mb-3">
                    <div>
                        ...
                        <input type="text" id="wiz-p1-name" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-emerald-500 focus:outline-none" placeholder="Player 1" />
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Rating (0-5)</label>
                        <input type="number" id="wiz-p1-rating" min="0" max="5" step="0.1" value="2.5" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-emerald-500 focus:outline-none" />
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3 mb-3">
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Player 2 Name</label>
                        <input type="text" id="wiz-p2-name" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-emerald-500 focus:outline-none" placeholder="Player 2" />
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Rating (0-5)</label>
                        <input type="number" id="wiz-p2-rating" min="0" max="5" step="0.1" value="2.5" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-emerald-500 focus:outline-none" />
                    </div>
                </div>
                <div class="mb-3">
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Team Name (optional)</label>
                    <input type="text" id="wiz-team-name" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-emerald-500 focus:outline-none" placeholder="Auto-generated if blank" />
                </div>
                <button onclick="addWizardTeam()" class="w-full px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white rounded-lg font-medium text-sm transition-colors">
                    + Add Team
                </button>
            </div>
            ...
            ${teams.length > 0 ? `
                ...
                <div class="text-center mt-2">
                    <span class="text-sm ${teams.length >= 4 ? 'text-emerald-600 font-semibold' : 'text-gray-500'}">${teams.length} team${teams.length !== 1 ? 's' : ''} added ${teams.length < 4 ? '(min 4 required)' : ''}</span>
                </div>
            ` : `
...
function renderCreateStep4() {
    return `
        <div id="wizard-content">
            ...
            <div class="space-y-3">
                <label class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${CreateWizard.accessMode === 'anyone' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-emerald-300'}" onclick="selectAccessMode('anyone')">
                ...
                <label class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors ${CreateWizard.accessMode === 'private' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 hover:border-emerald-300'}" onclick="selectAccessMode('private')">
...
function renderCreateStep5() {
    return `
            ...
            <div class="mt-6 bg-gray-50 rounded-xl p-4">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-800">${CreateWizard.tournamentName || 'Round Robin'}</span>
                    </div>
                    <div class="flex justify-between">
        ...
        <div class="flex gap-3 mt-6">
            ...
            <button onclick="createTournamentFromWizard()" class="flex-1 px-5 py-3 bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white rounded-xl font-semibold transition-colors">
                Create Tournament
            </button>
        </div>
    `;
            ...
            <div class="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-slide-up">
                ...
                <div class="p-6">
                    <div class="mb-6">
                        ...
                        <input type="text" id="join-code" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-emerald-500 focus:outline-none transition-colors text-lg text-center tracking-widest uppercase" placeholder="ABC123" maxlength="10" autofocus onkeypress="if(event.key === 'Enter') handleJoinTournament()" />
                    </div>
                    ...
                    <div class="flex gap-3">
                        <button onclick="closeModal()" class="flex-1 px-5 py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-medium transition-colors">Cancel</button>
                        <button onclick="handleJoinTournament()" class="flex-1 px-5 py-3 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl font-semibold transition-colors">Join</button>
                    </div>
                </div>
            </div>
        </div>
...
async function handleJoinTournament() {
    ...
    try {
        const snapshot = await database.ref(`roundrobin-tournaments/${code}/meta`).once('value');
...
console.log('✅ Round Robin Handlers loaded');
