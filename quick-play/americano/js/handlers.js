...
function goToWizardStep3() {
    ...
    if (!playerCountEl && window._createWizardData.playerCount) {
        // Continue with stored data - format already selected
        ...
        // If 24 players selected, ALWAYS go to format selection first
        if (playerCount === 24) {
            // Reset format type and go to selection
            window._createWizardData.formatType = null;
            goToWizardStep2b();
            return;
        }
    }
...
function showShareModal() {
    ...
    const shareButtonsHTML = typeof SocialShare !== 'undefined' ? SocialShare.getShareButtons(state.tournamentName || 'Americano', state.tournamentId, 'americano', playerLink, standings) : '';
...
async function handleOrganiserLogin() {
    ...
    try {
        ...
        if (verified) {
            ...
            try {
                sessionStorage.setItem('americano_organiser_' + state.tournamentId, '1');
...
/**
 * Move a round up in the order (decrease display index)
 */
function moveRoundUp(displayIndex) {
    if (!state || !state.canEdit()) return;
    if (displayIndex <= 0) return;
    
    state.moveRound(displayIndex, displayIndex - 1);
    render();
    showToast(`✅ Round moved up`);
}

/**
 * Move a round down in the order (increase display index)
 */
function moveRoundDown(displayIndex) {
    if (!state || !state.canEdit()) return;
    const rounds = state.getRounds();
    if (displayIndex >= rounds.length - 1) return;
    
    state.moveRound(displayIndex, displayIndex + 1);
    render();
    showToast(`✅ Round moved down`);
}
