...
function showShareLinksModal() {
    ...
    const shareButtonsHTML = typeof SocialShare !== 'undefined' ? SocialShare.getShareButtons(state.tournamentName || 'Mix Tournament', state.tournamentId, 'mix', playerLink, standings) : '';
...
/**
 * Move a round up in the order
 */
function moveRoundUp(displayIndex) {
    if (!state || !state.canEdit()) return;
    if (displayIndex <= 0) return;
    
    console.log('Moving round up:', displayIndex, '→', displayIndex - 1);
    console.log('Before:', state.roundOrder);
    state.moveRound(displayIndex, displayIndex - 1);
    console.log('After:', state.roundOrder);
    render();
    showToast('✅ Round moved up');
}

/**
 * Move a round down in the order
 */
function moveRoundDown(displayIndex) {
    if (!state || !state.canEdit()) return;
    if (displayIndex >= CONFIG.TOTAL_ROUNDS - 1) return;
    
    console.log('Moving round down:', displayIndex, '→', displayIndex + 1);
    console.log('Before:', state.roundOrder);
    state.moveRound(displayIndex, displayIndex + 1);
    console.log('After:', state.roundOrder);
    render();
...
function resetRoundOrder() {
    if (!state || !state.canEdit()) return;
    if (!confirm('Reset round order to default?')) return;
    
    state.resetRoundOrder();
    render();
