                ...
                ${hasCustomOrder ? `
                    <div class="bg-amber-50 border border-amber-200 rounded-lg p-4 flex items-center justify-between">
                        <div class="text-sm text-amber-700">
                            <span class="font-medium">Custom order active</span> - rounds have been reordered
                        </div>
                        <button onclick="resetRoundOrder()" class="px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-lg text-sm font-medium">
                            Reset to Default
                        </button>
                    </div>
                ` : ''}
                    ...
                    <div class="space-y-2">
                        ${Array.from({length: CONFIG.TOTAL_ROUNDS}, (_, displayIdx) => {
                            ...
                            const matchSummary = matches.slice(0, 2).map(m => {
                                ...
                                const t2 = m.team2.map(p => state.playerNames[p-1] || `P${p}`).join(' & ');
                                return `${t1} vs ${t2}`;
                            }).join(', ');
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    ...
                    ${state.roundOrder ? `
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
                            <p class="text-sm text-blue-800"><strong>📋 Custom Order Active:</strong> Rounds are shown in your custom order. Change order in the Rounds tab.</p>
                        </div>
                    ` : ''}
                    <div class="space-y-6">
                        ${roundOrder.map((originalIdx, displayIdx) => {
                            const round = originalIdx + 1;
                            ...
                            return `
                                <div class="border border-gray-200 rounded-lg p-4">
                                    <h4 class="font-semibold text-gray-700 mb-3">
                                        Round ${displayRound}
                                        ${state.roundOrder && originalIdx !== displayIdx ? 
                                            `<span class="text-xs text-gray-400 ml-1">(original R${round})</span>` : ''}
                                    </h4>
                                    <div class="space-y-3">
...
/**
 * TV Mode data adapter for Tournament format
 */
function getTvData() {
