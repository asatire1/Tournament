...
// Firebase config is safe to expose client-side — security is enforced by
// Firebase Security Rules, not by keeping these values secret.
// In Vite builds, set UBER_FIREBASE_* variables in your .env file (see .env.example)
// to override the defaults below. Never commit a .env with sensitive secrets.
...
const Firebase = {
    ...
    async saveScore(format, tournamentId, roundIndex, matchIndex, team1Score, team2Score) {
        try {
            ...
            // Note: meta/updatedAt is intentionally NOT written here.
            // Score writes are open to all users (players on court), but meta
            // writes require organiser ownership per Firebase Security Rules.
            return true;
        } catch (error) {
